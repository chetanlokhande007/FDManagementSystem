...
export class FDDetailComponent implements OnInit {
  ...
  loadFDDetails(id: number, forceRefresh: boolean = false, hideUI: boolean = true): Observable<any> {
    ...
    if (cachedData && !forceRefresh) {
      ...
      // When on the cashflow tab, the child FdCashflowComponent loads fresh
      // cash flow data independently — skip the parent's background fetch
      // to avoid duplicate API calls for the same endpoint.
  ...
  submitForApproval(): void {
    if (!this.fdId) return;
    if (confirm('Are you sure you want to submit this FD for approval? It will become read-only.')) {
      this.isSaving = true;
      this.fdService.submit(this.fdId).subscribe({
        next: (res) => {
          this.isSaving = false;
          alert('FD submitted for approval successfully.');
          this.loadFDDetails(this.fdId!).subscribe(); // Reload to update status and UI
        },
        error: (err) => {
          this.isSaving = false;
          alert(`Error submitting FD: ${err?.error?.message || err?.message || 'Unknown error'}`);
        }
      });
    }
  }
  ...
  save(): void {
    // Block save for approved/protected records
    if (this.isProtectedStatus) {
      this.showApprovedWarning = true;
      return;
    }
  ...
  get isApprovedStatus(): boolean {
    return this.fixedDeposit.status === 'APPROVED';
  }

  showApprovedWarning = false;

  get isApprovedReadOnly(): boolean {
