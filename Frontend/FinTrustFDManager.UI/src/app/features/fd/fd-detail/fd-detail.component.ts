...
export class FDDetailComponent implements OnInit {
  ...
  getCurrencyName(id: number | null): string {
    if (!id) return '';
    const cur = this.currencies.find(x => x.currencyId === id);
    return cur ? cur.currencyCode + ' - ' + cur.currencyName : '';
  }
  ...
  validateField(field: string): void {
    if (field === 'entityId') {
      ...
      if (!this.fixedDeposit.currencyId) this.errors.currencyId = 'Transaction Currency is required.';
      else delete this.errors.currencyId;
    }
  ...
  get isProtectedStatus(): boolean {
    ...
    return s === 'APPROVED' || s === 'ACTIVE' || s === 'MATURED'
        || s === 'PENDING_FD_ADMIN' || s === 'PENDING_CA';
  }
  ...
  submitForApproval(): void {
    if (!this.fdId) return;
    const confirmed = confirm('Submit this FD for approval?');
    if (!confirmed) return;

    this.fdService.submit(this.fdId).subscribe({
      next: () => {
        this.fixedDeposit.status = 'PENDING_FD_ADMIN';
        this.isGeneralReadOnly = true;
        alert('FD submitted for approval.');
      },
      error: (err: any) => {
        alert(`Submit Error: ${err?.error?.message || err?.message || 'Unable to submit FD.'}`);
      }
    });
  }
