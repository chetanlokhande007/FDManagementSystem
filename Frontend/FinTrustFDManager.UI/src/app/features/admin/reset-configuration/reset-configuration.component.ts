...
@Component({
  ...
  styleUrl: './reset-configuration.component.css'
})
export class ResetConfigurationComponent implements OnInit {
  ...
  loadFDs(): void {
    ...
    this.fdService.getLandingData().subscribe({
      ...
      error: (err) => {
        console.error('Error loading FDs for reset configuration', err);
        this.loading = false;
      }
    });
  }
  ...
  goToFD(fd: FDLanding): void {
    this.router.navigate(['/fd-detail', fd.fdId], { queryParams: { tab: 'interest' } });
  }
